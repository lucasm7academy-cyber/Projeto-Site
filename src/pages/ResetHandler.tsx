...
export default function ResetPassword() {
  ...
  useEffect(() => {
    const processHash = async () => {
      // Verificar se tem hash na URL (token do email)
      const hash = window.location.hash;
      
      if (hash && hash.includes('access_token')) {
        // O Supabase já processa automaticamente o hash
        // Aguardar um momento para o Supabase criar a sessão
        ...
        // Sem hash, verificar se já tem sessão (caso alguém tente acessar direto)
        const { data: { session } } = await supabase.auth.getSession();
    ...
    return (
      <div className="relative min-h-screen w-full flex items-center justify-center p-4 overflow-hidden">
        ...
        <div className="relative z-10 text-center">
          <div className="text-primary text-xl animate-pulse">Processando link de recuperação...</div>
        </div>
      </div>
    );
  }
